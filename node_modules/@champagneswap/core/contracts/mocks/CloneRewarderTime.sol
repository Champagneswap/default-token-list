...
contract CloneRewarderTime is IRewarder,  BoringOwnable{
    ...
    function onChamReward (uint256 pid, address _user, address to, uint256, uint256 lpTokenAmount) onlyMCV2 lock override external {
        require(IWineMakerV2(MASTERCHEF_V2).lpToken(pid) == masterLpToken);
    ...
    /// @param _rewardPerSecond The amount of Champagne to be distributed per second.
    ...
    function pendingToken(uint256 _pid, address _user) public view returns (uint256 pending) {
        ...
        uint256 lpSupply = IWineMakerV2(MASTERCHEF_V2).lpToken(_pid).balanceOf(MASTERCHEF_V2);
        if (block.timestamp > pool.lastRewardTime && lpSupply != 0) {
            uint256 time = block.timestamp.sub(pool.lastRewardTime);
            uint256 chamReward = time.mul(rewardPerSecond);
            accToken1PerShare = accToken1PerShare.add(chamReward.mul(ACC_TOKEN_PRECISION) / lpSupply);
        }
    ...
    function updatePool(uint256 pid) public returns (PoolInfo memory pool) {
        ...
        if (block.timestamp > pool.lastRewardTime) {
            uint256 lpSupply = IWineMakerV2(MASTERCHEF_V2).lpToken(pid).balanceOf(MASTERCHEF_V2);

            if (lpSupply > 0) {
                uint256 time = block.timestamp.sub(pool.lastRewardTime);
                uint256 chamReward = time.mul(rewardPerSecond);
                pool.accToken1PerShare = pool.accToken1PerShare.add((chamReward.mul(ACC_TOKEN_PRECISION) / lpSupply).to128());
            }
