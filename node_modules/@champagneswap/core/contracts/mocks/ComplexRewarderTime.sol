...
contract ComplexRewarderTime is IRewarder,  BoringOwnable{
    ...
    function onChamReward (uint256 pid, address _user, address to, uint256, uint256 lpToken) onlyMCV2 lock override external {
        PoolInfo memory pool = updatePool(pid);
        UserInfo storage user = userInfo[pid][_user];
        uint256 pending;
        if (user.amount > 0) {
            pending =
                (user.amount.mul(pool.accChamPerShare) / ACC_TOKEN_PRECISION).sub(
                    user.rewardDebt
        ...
        user.amount = lpToken;
        user.rewardDebt = lpToken.mul(pool.accChamPerShare) / ACC_TOKEN_PRECISION;
    ...
    /// @param _rewardPerSecond The amount of Champange to be distributed per second.
    ...
    function pendingToken(uint256 _pid, address _user) public view returns (uint256 pending) {
        ...
        uint256 lpSupply = WineMakerV2(MASTERCHEF_V2).lpToken(_pid).balanceOf(MASTERCHEF_V2);
        if (block.timestamp > pool.lastRewardTime && lpSupply != 0) {
            uint256 time = block.timestamp.sub(pool.lastRewardTime);
            uint256 chamReward = time.mul(rewardPerSecond).mul(pool.allocPoint) / totalAllocPoint;
            accChamPerShare = accChamPerShare.add(chamReward.mul(ACC_TOKEN_PRECISION) / lpSupply);
        }
        pending = (user.amount.mul(accChamPerShare) / ACC_TOKEN_PRECISION).sub(user.rewardDebt).add(user.unpaidRewards);
    }
    ...
    function updatePool(uint256 pid) public returns (PoolInfo memory pool) {
        ...
        if (block.timestamp > pool.lastRewardTime) {
            uint256 lpSupply = WineMakerV2(MASTERCHEF_V2).lpToken(pid).balanceOf(MASTERCHEF_V2);

            if (lpSupply > 0) {
                uint256 time = block.timestamp.sub(pool.lastRewardTime);
                uint256 chamReward = time.mul(rewardPerSecond).mul(pool.allocPoint) / totalAllocPoint;
                pool.accChamPerShare = pool.accChamPerShare.add((chamReward.mul(ACC_TOKEN_PRECISION) / lpSupply).to128());
            }
